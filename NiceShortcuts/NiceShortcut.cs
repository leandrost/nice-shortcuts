...
{
	/// <summary>
	/// This Class is based on HotKey from this article: http://bloggablea.wordpress.com/2007/05/01/global-hotkeys-with-net/
	/// </summary>
	public class NiceShortcut : IMessageFilter
	{
		#region Interop
		...
		//Methods
		public NiceShortcut Clone()
		{
			// Clone the whole object
			return new NiceShortcut 
			{ 
		...
		public bool Register(Control windowControl)
        {
			...
			this.id = NiceShortcut.currentID;
			NiceShortcut.currentID = NiceShortcut.currentID + 1 % NiceShortcut.maximumID;

			// Translate modifier keys into unmanaged version
			uint modifiers = (this.Alt ? NiceShortcut.MOD_ALT : 0) | (this.Control ? NiceShortcut.MOD_CONTROL : 0) |
							(this.Shift ? NiceShortcut.MOD_SHIFT : 0) | (this.Windows ? NiceShortcut.MOD_WIN : 0);

			// Register the hotkey
			if (NiceShortcut.RegisterHotKey(windowControl.Handle, this.id, modifiers, keyCode) == 0)
			{ 
		...
		public void Unregister()
		{
			...
			if (!this.windowControl.IsDisposed)
			{
				...
				if (NiceShortcut.UnregisterHotKey(this.windowControl.Handle, this.id) == 0)
				{ throw new Win32Exception(); }
			}
		...
		public bool PreFilterMessage(ref Message message)
		{
			...
			if (message.Msg != NiceShortcut.WM_HOTKEY)
			{ return false; }
		...
		private void SetHotKey(string shortcut)
		{
			var keys = shortcut.ToUpper().Split('+');
